...
For fixing "more" pages, it adds anchor #n to the link that tells us the page's location from homepage. When a page is loaded with "unknown or expired link", it goes back to homepage and fetches until the nth page. (Note: Due to the way voting works links are constantly rearranged and for example the 3rd page now might have links you have just read in the 2nd page) 
...
The new comment button refreshes this page in background, fetches the new fnid and updates the current page's form. Then submits the comment. Since this is a little slower, you can always try the regular "comment" button first and if it doesn't work, hit back button and then hit the new comment button.